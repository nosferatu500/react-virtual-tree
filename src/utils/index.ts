...
import { IndividualTreeViewState, VirtualForestProps, TreeItem, TreeItemActions, TreeItemIndex, TreeItemRenderContext, TreeItemRenderFlags, VirtualTreeContextProps } from "../types";
...
export const createTreeItemRenderContext = <T>(item: TreeItem<T>, context: VirtualTreeContextProps, treeId: string): TreeItemRenderContext => {
    const viewState = context.viewState[treeId];

    const canDrag = (viewState.selectedItems?.length ?? 0) > 0 ? (viewState.selectedItems
        ?.map(item => context.items[item]?.canMove)
    ...
    const actions: TreeItemActions = {
        ...
        startDragging: () => {
            ...
            if (canDrag) {
                context.onStartDragItems((viewState.selectedItems ?? []).map(id => context.items[id]), treeId);
            }
        }
    }
    ...
    const renderContext: TreeItemRenderFlags = {
        ...
        isDraggingOver:
            ...
            context.draggingPosition.childIndex === undefined,
...
export const createTreeItemRenderContextDependencies = <T>(item: TreeItem<T> | undefined, context: VirtualForestProps, treeId: string) => [
    context,
    context.viewState.expandedItems,
